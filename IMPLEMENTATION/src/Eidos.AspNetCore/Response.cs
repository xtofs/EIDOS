...
/// <summary>
/// Non-generic view of an <see cref="Response{T}"/>, used by the route builder to enrich and serialize
...
/// a <c>Location</c> remain controllable. Use the <see cref="Response"/> factory methods to construct one.
/// </summary>
public sealed class Response<T> : IResult, IEidosRepresentation
{
    private readonly List<KeyValuePair<string, string>> _headers = [];

    internal Response(T? value, int statusCode, string? location, IResult? passThrough)
    {
        Value = value;
    ...
    public Response<T> WithHeader(string name, string value)
    {
    ...
    /// <summary>
    /// Safety net for when an <see cref="Response{T}"/> is executed directly rather than through the Eidos
...
/// <summary>Factory methods for <see cref="Response{T}"/>.</summary>
public static class Response
{
    /// <summary>200 OK carrying a representation (single value or a collection).</summary>
    public static Response<T> Ok<T>(T value) => new(value, StatusCodes.Status200OK, location: null, passThrough: null);
    ...
    public static Response<T> BadRequest<T>(object? error = null) =>
    ...
    public static Response<T> Problem<T>(string? detail = null, int statusCode = StatusCodes.Status400BadRequest) =>
    ...
    public static Response<T> FromResult<T>(IResult result) => new(default, StatusCodes.Status200OK, null, result);
}
